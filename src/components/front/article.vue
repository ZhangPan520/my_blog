      ...
      <div class="indexes animated fadeIn">
        <div class="last animated fadeIn">
          ...
          >
            ...
            >{{ articles[prePage].title }}</router-link
          >
          <router-link
            :to="{
              name: 'article',
              params: {
                id: articles[nextPage].aid,
                index: nextPage,
                page: $route.params.page
              },
              hash: '#article'
            ...
            class="right"
          >
            {{ articles[nextPage].title }}<i class="iconfont icon-right"></i
          ></router-link>
        </div>
      </div>
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    } else if (to.hash && to.hash !== "#article") {
      // 目录锚点跳转-
  ...
  computed: {
    ...
    mdHtml() {
      console.log(marked(this.article.content,{ renderer: renderer }))
