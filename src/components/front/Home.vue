    ...
    <div class="container">
        <section class="newBlog">
            ...
            <div class="posts animated fadeIn">
                <div class="flex">
                    <div v-for="(article, index) in reducedArticles" class="oneArticle" :key='article.aid'>
                        <div class="option">
...
export default {
    ...
    created () {
        this.visible_people().then(()=>{
            this.get_watch().then(res=>{
                this.$store.state.visible_people=res.body[0].count
            })
        })
        this.set_headline({
    ...
    methods: {
        ...
        ...mapActions(['getAllArticles', 'sendMail','visible_people','get_watch']),
        send () {
            const re = /^[\w_-]+@[\w_-]+\.[\w\\.]+$/
