...
export default {
    login ({commit}, payload) {
        return Vue.http.post('/api/login', payload).catch((err) => { throw new Error(err.data) })
    },
    ...
    comment_tongguo({},payload){
        console.log(payload.id)
        return Vue.http.post('/api/comment_tongguo',{params:{payload}})
    },
    ...
    deleteComment({commit},payload){
        return Vue.http.delete('/api/delete',{params:{payload}})
    },
    updateLike ({commit}, payload) {
        return Vue.http.patch('/api/comments/' + payload.id, {option: payload.option})
                .catch((err) => { console.log(err) })
    },
    getLiuyans({commit},payload){
        return Vue.http.get('/api/liuyans', {params: {payload}})
                .then(response => response.json())
                .then(liuyans => {
                    commit('set_liuyans', liuyans)
                }).catch((err) => { console.log(err) })
    },
    deleteLiuyan({commit},payload){
        return Vue.http.delete('/api//delete_liuyan',{params:{payload}})
    },
    summitLiuyan ({commit}, payload) {
        return Vue.http.post('/api/liuyan', payload)
    },
    liuyan_tongguo({},payload){
        console.log(payload.id)
        return Vue.http.post('/api/liuyan_tongguo',{params:{payload}})
    },
    visible_people(){
        return Vue.http.post('/api/visible')
    },
    get_watch(){
        return Vue.http.get('/api/countwatch')
    },
    //发送验证码
    sendCode(comment,payload){
        return Vue.http.get('api/code',{params:payload})
    }
}
