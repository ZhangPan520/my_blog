...
const liuyan = resolve => require(['@/components/front/liuyan'], resolve)
const liuyanguanli = resolve => require(['@/components/back/liuyan'], resolve)
Vue.use(Router)
    ...
    routes: [
        {
            ...
            children: [
                ...
                {path:'liuyan',name:"liuyan",component:liuyan,meta:{title:"留言"}}
            ]
        },
        {
            path: '/login',
        ...
        {path: 'liuyanguanli', name: 'liuyanguanli', component: liuyanguanli, meta: {requireAuth: true, title: '留言管理'}}
    ]
})
...
router.beforeEach((to, from, next) => {
    ...
    if (localStorage.getItem('token') && to.name === 'login') {
        next({name: 'posts'})
    } else if (!localStorage.getItem('token') && to.meta.requireAuth) {
        next({name: 'login'})
    } else {
        next()
    }
