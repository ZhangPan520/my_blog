...
router.delete('/api/delete',(req,res)=>{
    ...
    db.Comment.remove({_id: _id}, (err, data) => {
        if (err) {
...
// 评论审核通过
router.post('/api/comment_tongguo',(req,res)=>{
    const {_id} = req.body.params.payload
    var fix = {isPublish:true,_id};
    console.log(fix)
    // new db.Comment(fix).save();
    // new db.comment({isPublish}).save()
    db.Comment.update({_id: _id}, {isPublish:true}, (err, data) => {
        if (err) {
...
router.get('/api/comments', (req, res) => {
    ...
    if (req.query.payload.sort === 'date') {                                // 根据时间排序评论
        if(role=='false'){
            db.Comment.find({$and:[{ articleId: articleId},{isPublish:true}]}, 'name date content like imgName isPublish').sort({ date: -1 }).exec()
            .then((comments) => {
                res.send(comments)
            })
        }else{
            db.Comment.find({ articleId: articleId}, 'name date content like imgName isPublish').sort({ date: -1 }).exec()
            .then((comments) => {
                res.send(comments)
            })
        }
        ...
        if(role=='false'){
            db.Comment.find({$and:[{ articleId: articleId},{isPublish:true}]}, 'name date content like imgName isPublish').sort({ like: -1 }).exec()
            .then((comments) => {
                res.send(comments)
            })
        }else{
            db.Comment.find({ articleId: articleId }, 'name date content like imgName isPublish').sort({ like: -1 }).exec()
            .then((comments) => {
                res.send(comments)
            })
        }
        ...
        if (role=='false') {
            db.Comment.find({$and:[{ articleId: articleId},{isPublish:true}]}, 'name date content like imgName isPublish').exec().then((comments) => {
                res.send(comments)
            })
        }else{
            db.Comment.find({ articleId: articleId}, 'name date content like imgName isPublish').exec().then((comments) => {
                res.send(comments)
            })
        }
    }
